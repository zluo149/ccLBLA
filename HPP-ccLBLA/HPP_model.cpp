...
void model::estimate() {
    ...
    save_model("new-Model"); //utils::generate_model_name(-1)
    printf("saved model\n");
...
int model::sampling(int m, int n) {
    ...
    double noise = laplaceRandom(0, 1/epsilonL);
    ...
    double tempNwk = min((nw[w][topic] + noise), ClipBound);
    double tempNcwk = min((nwc[c][w][topic] + noise), ClipBound);
    ...
    if (route == 0) {
      ...
      for (int k = 0; k < K; k++) {
        prob = ((tempNwk + pv[w]) / (hps + hpt + nwsum[k]) * (hps + nwsum[k] - nw[V-1][k])/(countS + nwsum[k] - nw[V-1][k])) *
            ((nd[m][k] + pu[c][k]) / (hpu + hpv + ndsum[m]) *constant);
        p[k] = prob;
        totalprob += prob;
      }
      ...
      for (int k = 0; k < K; k++) {
        ...
        prob = ((tempNcwk + pv[w]) / (hps + hpt + nwcsum[c][k]) * (hps + nwcsum[c][k] - nwc[c][V-1][k]) / (countS + nwcsum[c][k] - nwc[c][V-1][k])) * ///
    ...
    prob0 = ((tempNwk + pv[w]) / (hps + hpt + nwsum[topic]) * (hps + nwsum[topic] - nw[V-1][topic]) / (countS + nwsum[topic] - nw[V-1][topic])) *
                 (nx[0][c][topic] + gamma0) / (nxsum[c][topic] + gamma0 + gamma1);

    prob1 = ((tempNcwk + pv[w]) / (hps + hpt + nwcsum[c][topic]) * (hps + nwcsum[c][topic] - nwc[c][V-1][topic]) / (countS + nwcsum[c][topic] - nwc[c][V-1][topic])) *
...
int model::inf_sampling(int m, int n) {
    ...
    if (route == 0) {
      for (int k = 0; k < K; k++) {
        ...
        prob = ((nw[w][k] + newnw[_w][k] + beta) / (nwsum[k] + newnwsum[k] + Vbeta)) *
      ...
      for (int k = 0; k < K; k++) {
        ...
        prob = ((nwc[c][w][k] + newnwc[c][_w][k] + beta) / (nwcsum[c][k] + newnwcsum[c][k] + Vbeta)) *
            ((newnd[m][k] + pu[c][k]) / (hpu + hpv + newndsum[m]) *constant);
   ...
   prob0 = ((nw[w][topic] + newnw[_w][topic] + beta) / (nwsum[topic] + newnwsum[topic] + Vbeta)) *
	        ((nx[0][c][topic] + gamma0) / (nxsum[c][topic] + gamma0 + gamma1));

   prob1 = ((nwc[c][w][topic] + newnwc[c][_w][topic] + beta) / (nwcsum[c][topic] + newnwcsum[c][topic] + Vbeta)) *
	        ((nx[1][c][topic] + gamma1) / (nxsum[c][topic] + gamma0 + gamma1));
    ...
    for (int k = 0; k < K; k++) {
	     for (int w = 0; w < newV; w++) {
          ...
          for (int c = 0; c < C; c++) {
             ...
             newsigma[c][k][w] = (nwc[c][it->second][k] + newnwc[c][w][k] + beta) / (nwcsum[c][k] + newnwcsum[c][k] + V * beta);
          }
	    }
    }
}
...
double model::laplaceRandom(double m, double b) {
	double v = 0;
